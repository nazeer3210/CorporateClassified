...
export class OfferDetailsComponent implements OnInit {
  ...
  offer: Offer = new Offer(0, "offer name", "offer details", "category", new Date(), new Date(), new Date(), 0)
  ...
  ngOnInit(): void {
    ...
    if (this.token != null && this.offerId != 0)
      this.configService.getOfferDetailsById(this.token, this.offerId).subscribe((data: Offer) => {
  ...
  submitLike() {
    if (this.token != null)
      this.configService.saveLike(this.token, this.offerId).subscribe((data) => {
  ...
  engageUser() {
    //call the rest api
    if (this.token != null) {
      this.configService.engageOffer(this.token, this.offer.offerId, this.empId).subscribe((data: messageResponse) => {
